...
void loop() {
  ...
  if (dist_filtered < 180) {
    pulse_width = _DUTY_MIN;  // 0도 (500µs)
  } else if (dist_filtered > 360) {
    pulse_width = _DUTY_MAX;  // 180도 (2500µs)
  } else {
    // 거리 180mm ~ 360mm 사이에서 서보 각도에 맞게 마이크로초 단위로 펄스 조정
    pulse_width = map(dist_filtered, 180, 360, _DUTY_MIN, _DUTY_MAX);
  }
